...
router.post('/', authenticate, [
  ...
  try {
    ...
    const result = await db.query(
      `INSERT INTO adoption_applications (user_id, pet_id, full_name, email, phone_number,
        home_address, previous_pet_experience, why_adopt, why_chosen)
       VALUES ($1,$2,$3,$4,$5,$6,$7,$8,$9) RETURNING *`,
      [req.user.user_id, pet_id, full_name, email, phone_number, home_address,
       previous_pet_experience, why_adopt, why_chosen]
    );
    // Update pet status to Pending
    await db.query("UPDATE available_pets SET status = 'Pending', updated_at = NOW() WHERE pet_id = $1", [pet_id]);
    // Send notification
    await db.query(
      `INSERT INTO notifications (user_id, type, title, message, related_id)
       VALUES ($1, 'application_received', $2, $3, $4)`,
      [req.user.user_id, 'Application Received',
       `Your adoption application for ${petCheck.rows[0].pet_name} has been received and is under review.`,
...
router.get('/my-applications', authenticate, async (req, res) => {
  try {
    const result = await db.query(
      `SELECT aa.*, ap.pet_name, ap.pet_type, ap.breed, ap.photos,
              s.shelter_name
...
// PUT /api/adoptions/:id/review - shelter admin review
router.put('/:id/review', authenticate, [
  ...
  try {
    ...
    const result = await db.query(
      `UPDATE adoption_applications SET status = $1, notes = $2, reviewed_at = NOW(), reviewed_by = $3, updated_at = NOW()
       WHERE application_id = $4 RETURNING *, (SELECT pet_id FROM adoption_applications WHERE application_id = $4)`,
    ...
    if (status === 'Approved') {
      await db.query("UPDATE available_pets SET status = 'Adopted', adopted_date = NOW() WHERE pet_id = $1", [app.pet_id]);
    } else if (status === 'Rejected') {
      await db.query("UPDATE available_pets SET status = 'Available', updated_at = NOW() WHERE pet_id = $1", [app.pet_id]);
    }
    // Notify applicant
    const notifMsg = status === 'Approved'
      ? 'Congratulations! Your adoption application has been APPROVED!'
      : status === 'Rejected'
      ? `Your adoption application has been rejected. ${notes || ''}`
      : 'Your application is under review.';
    await db.query(
      `INSERT INTO notifications (user_id, type, title, message, related_id)
       VALUES ($1, 'application_status', $2, $3, $4)`,
      [app.user_id, `Application ${status}`, notifMsg, app.application_id]
    );
