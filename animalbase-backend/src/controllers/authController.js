...
const sendOtpEmail = async ({
  ...
  await transporter.sendMail({
    ...
    text: [
      ...
      `${codeLabel}: ${otp}`,
      '',
      `This code will expire in ${expiresInText}.`,
    ...
    html: `
      <div style="font-family: Arial, sans-serif; max-width: 520px; margin: 0 auto; padding: 24px; color: #1f2937;">
        ...
        <p style="font-size: 14px; color: #6b7280;">This code expires in ${expiresInText}. If you did not request this code, you can ignore this email.</p>
      </div>
    `,
  });
};
...
const savePendingOtp = async (email, otp, ttlMs) => {
  await pool.query(
...
const markEmailVerified = async (email, ttlMs) => {
  await pool.query(
...
const sendOtp = async (req, res) => {
  try {
    ...
    const otp = createOtp();
    await savePendingOtp(email, otp, SIGNUP_OTP_TTL_MS);

    try {
      await sendSignupOtpEmail(email, otp);
    } catch (mailErr) {
      console.error('[auth] Unable to send signup OTP email:', mailErr);
      throw mailErr;
    }

    return res.json({
      message: 'Verification code sent.',
      expiresInSeconds: SIGNUP_OTP_TTL_MS / 1000,
    });
  } catch (err) {
...
const sendResetOtp = async (req, res) => {
  try {
    ...
    const otp = createOtp();
    await savePendingOtp(email, otp, RESET_OTP_TTL_MS);

    try {
      await sendPasswordResetOtpEmail(email, otp);
    } catch (mailErr) {
      console.error('[auth] Unable to send password reset OTP email:', mailErr);
      throw mailErr;
    }
    ...
    return res.json({
      ...
      expiresInSeconds: RESET_OTP_TTL_MS / 1000,
    });
  } catch (err) {
...
const verifyOtp = async (req, res) => {
  try {
    ...
    await markEmailVerified(email, VERIFIED_SIGNUP_TTL_MS);
...
const deleteMe = async (req, res) => {
  try {
    ...
    const passwordMatches = await bcrypt.compare(password, passwordResult.rows[0].password);
    if (!passwordMatches) {
      return res.status(400).json({ error: 'Current password is incorrect.' });
    }

    const result = await deleteUserAccountById(req.user.id);
    ...
    return res.json({
      message:
        'Account deleted. Your reports were removed, and your adoption history was detached from this account.',
    });
  } catch (err) {
