...
const updateUser = async (req, res) => {
  try {
    ...
    const result = await pool.query(
      `UPDATE users SET full_name=$1, email=$2, phone=$3, address=$4, role=$5
       WHERE id=$6
       RETURNING id, full_name, email, phone, address, avatar_url, role, joined_at`,
      [fullName, email, phone, address, role, req.params.id]
    );
    if (result.rows.length === 0) return res.status(404).json({ error: 'User not found.' });
    const u = result.rows[0];
    res.json({
...
const updateApplicationStatus = async (req, res) => {
  try {
    ...
    if (status === 'Approved') {
      await pool.query("UPDATE pets SET status='Adopted' WHERE id=$1", [result.rows[0].pet_id]);
    }
